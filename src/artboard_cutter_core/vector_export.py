...
def export_artboards_vector_uniform(
    ...
    for idx, panel in enumerate(panel_layout):
        ...
        # The source clip is expressed in the source page coordinate space.
        # Stretch mode intentionally uses independent X/Y scale factors so vector
        # export matches raster's user-requested width and height geometry.
        ...
        out_page.show_pdf_page(out_page.rect, src_doc, page_index, clip=clip_src, keep_proportion=(fit_mode != "stretch"))
        out_name = f"{base_name}_{idx+1}.pdf"
        out.save(outdir / out_name)
        out.close()

        if log_cb:
            scale_txt = f"sx={sx:.6f} sy={sy:.6f}" if fit_mode == "stretch" else f"scale={sx:.6f}"
            log_cb(
                f"[CROP] {out_name} (VECTOR {fit_mode.upper()}): x=[{pt_to_mm(x0_t):.3f},{pt_to_mm(x1_t):.3f}] mm  "
                f"w={pt_to_mm(w_t):.3f} mm  h={pt_to_mm(h_t):.3f} mm  {scale_txt}"
            )
...
def _export_artboards_vector_stretch_from_master(
    ...
    try:
        ...
        for idx, panel in enumerate(panel_layout):
            ...
            if log_cb:
                log_cb(
                    f"[CROP] {out_name} (VECTOR STRETCH): x=[{pt_to_mm(x0_t):.3f},{pt_to_mm(x1_t):.3f}] mm  "
